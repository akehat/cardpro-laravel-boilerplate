...
class subscription_amounts_live extends Model
{
...
public static function authenticateSearch($api_userID, $api_key, $search)
{
    ...
    use HasFactory;
    protected $table="subscription_amounts_live";
    protected $guarded=['id'];
    public static $name='subscription_amounts';
    public static function updateFromIds_live($sub_id,$amount_id){
        self::fromArray([json_decode(subscriptionController::fetchSubscriptionAmount(config("app.api_username"),config("app.api_password"),$sub_id,$amount_id,'https://finix.live-payments-api.com')[0])]);
     }
    public static function runUpdateWithID($sub_id){
        $result= subscriptionController::listSubscriptionAmounts(config("app.api_username"),config("app.api_password"),$sub_id,'https://finix.live-payments-api.com');
        ...
        while(isset($object->_embedded)&&isset($object->_embedded->subscription_amounts)&&isset($object->page)&&isset($object->page->next_cursor)&&count($object->_embedded->subscription_amounts)>0){
         ...
         $result= subscriptionController::listSubscriptionAmounts(config("app.api_username"),config("app.api_password"),$sub_id,'https://finix.live-payments-api.com',$nextArray);
         $object=json_decode($result[0]);
        }
     }
