...
class Dispute_Evidence extends Model
{
    use HasFactory;
...
public static function updateFromIds($id,$evidenceid){
   self::fromArray([json_decode(merchantsController::fetchEvidenceAboutDispute(config("app.api_username"),config("app.api_password"),$id,$evidenceid)[0])]);
}
public static function runUpdateWithID($id){
    $result= merchantsController::listEvidenceAboutDispute(config("app.api_username"),config("app.api_password"),$id);
    ...
    while(isset($object->_embedded)&&isset($object->_embedded->evidences)&&isset($object->page)&&isset($object->page->next_cursor)&&count($object->_embedded->evidences)>0){
     ...
     $result= merchantsController::listEvidenceAboutDispute(config("app.api_username"),config("app.api_password"),'https://finix.sandbox-payments-api.com',$nextArray);
     $object=json_decode($result[0]);
    }
 }
